...
export async function run(projectRoot: string, argv: string[]): Promise<void> {
  ...
  if (flags.help) {
    console.log(`
llmsify — generate AI-readable markdown from your Next.js source

Usage: npx llmsify [options]
...
`);
  ...
  if (skipped.length > 0) {
    for (const f of skipped) {
      console.warn(`llmsify: skipped dynamic route: ${path.relative(projectRoot, f)}`);
    }
  }
  ...
  console.log(
    `llmsify: generated ${results.length} pages + llms.txt → ${path.relative(projectRoot, outDir)}/`
  );
  ...
  console.log('llmsify: annotation guide → ai-annotation-guide.md');
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  run(process.cwd(), process.argv.slice(2)).catch((err) => {
    console.error('llmsify:', err.message);
    process.exit(1);
  });
}

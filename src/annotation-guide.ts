...
async function readAgentifyIgnore(projectRoot: string): Promise<string[]> {
  try {
    const raw = await fs.readFile(path.join(projectRoot, '.llmsifyignore'), 'utf-8');
    return raw
      .split('\n')
      .map(l => l.trim())
...
export function renderAnnotationGuide(files: FileAnnotations[]): string {
  const lines: string[] = [
    ...
    '',
    'Generated by [llmsify](https://github.com/ggange/agentify). This file identifies',
    'content in your codebase that would benefit from explicit `<AIContent>` annotation,',
    'so llmsify can extract it into AI-readable markdown.',
    '',
    '## How it works',
    '',
    '`<AIContent>` (from `@pkg/react`) is a React Fragment wrapper — **zero DOM elements,',
    'zero layout impact.** It marks content for llmsify to include in per-route `.md` files.',
    'Once you annotate, re-run `npx llmsify` and your previously-empty pages will have content.',
    '',
    ...
    '> no DOM wrapper, no layout change. After annotating, re-run `npx llmsify`."',
    '',
    '> **Note:** this file is regenerated on every `npx llmsify` run — edits are ephemeral.',
    '> To permanently exclude files, add glob patterns to `.llmsifyignore` in your project root',

...
export function renderAnnotationGuide(files: FileAnnotations[]): string {
  const lines: string[] = [
    ...
    '',
    'Generated by [telogen](https://github.com/ggange/telogen). This file identifies',
    'content in your codebase that would benefit from explicit `<AIContent>` annotation,',
    'so telogen can extract it into AI-readable markdown.',
    '',
    '## How it works',
    '',
    '`<AIContent>` (from `@pkg/react`) is a React Fragment wrapper — **zero DOM elements,',
    'zero layout impact.** It marks content for telogen to include in per-route `.md` files.',
    'Once you annotate, re-run `npx telogen` and your previously-empty pages will have content.',
    '',
    '## Status: `@pkg/react` is not yet published',
    '',
    '**Do not run the prompt below yet.** `@pkg/react` is in progress and will be published soon.',
    'Watch [github.com/ggange/telogen](https://github.com/ggange/telogen) for the release.',
    ...
    '> no DOM wrapper, no layout change. After annotating, re-run `npx telogen`."',
    '',
